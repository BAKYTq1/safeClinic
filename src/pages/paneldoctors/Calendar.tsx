            ...
            <div className='calendar-main'>
                ...
                <div className='patient-time'>
                    {appointmentTimes.map((time, index) => (
                        <div key={index}>
                            {(index % 2 === 0 ? [0, 1, 2] : [0, 1, 2]).map((cardIdx) => {
                                const uniqueId = `${index}-${cardIdx}`;
                                const content = (
                                    <div className='patient-cards' key={uniqueId}>
                                        <img
                                            className='img'
                                            src={img27}
                                            alt=""
                                ...
                                return index % 2 === 0
                                    ? <React.Fragment key={uniqueId}>{content}<div className='empty'></div></React.Fragment>
                                    : <React.Fragment key={uniqueId}><div className='empty'></div>{content}</React.Fragment>;
                            })}
                        </div>
                    ))}
                </div>
